...
tasks.set('clean', () => Promise.resolve()
  .then(() => del(['coverage/*', 'wwwroot/dist', 'GAF.MyRoof.com/bin/*'], { dot: true }))
);
...
tasks.set('appsettings', () => new Promise(resolve => {
  ...
  const source = require('./GAF.MyRoof.com/appsettings.json');
  delete source.Logging;
  environments.forEach(env => {
    const filename = path.resolve(__dirname, `./GAF.MyRoof.com/appsettings.${env}.json`);
    try {
...
tasks.set('build', () => {
  ...
  return Promise.resolve()
    ...
    .then(() => new Promise((resolve, reject) => {
      ...
      let args = ['publish', 'GAF.MyRoof.com', '-o', '../build', '-c', config];
      if (global.DEBUG) args = ['build', 'GAF.MyRoof.com', '-c', config];
  ...
  return Promise.resolve()
    ...
    .then(() => new Promise(resolve => {
      ...
      compiler.hooks.done.tap('done', () => {
        ...
        if (++count === 1) {
          const options = {
            cwd: path.resolve(__dirname, './GAF.MyRoof.com/'),
